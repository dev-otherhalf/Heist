// =============================================================================
// Coffee standards — rolls the annotation digits ("24724 1632 248") up to their
// final values the first time the section scrolls into view. Each digit gets a
// taller strip than the one before it so they land in a staggered cascade.
// =============================================================================

class CoffeeStandards {
  constructor(section) {
  ...
  init() {
    if (!this.annotation || this.digits.length === 0) return;
...
const mount = (section) => {
  ...
  const instance = new CoffeeStandards(section);
  instance.init();
...
document.querySelectorAll("[data-coffee-standards]").forEach(mount);

document.addEventListener("shopify:section:load", (event) => {
  event.target.querySelectorAll("[data-coffee-standards]").forEach(mount);
});

document.addEventListener("shopify:section:unload", (event) => {
  event.target.querySelectorAll("[data-coffee-standards]").forEach(unmount);
});
