...
export class CartItemsComponent extends createViewEventElement(Component) {
  ...
  #handleNoteUpdate = (event) => {
    ...
    event.promise
      ?.then(({ detail }) => {
        const sections = /** @type {Record<string, string> | undefined} */ (
          detail?.sections
        );
  ...
  connectedCallback() {
    ...
    document.addEventListener(
      StandardEvents.cartDiscountUpdate,
      this.#handleDiscountUpdate,
    );
    document.addEventListener(
      StandardEvents.cartNoteUpdate,
  ...
  updateQuantity(config) {
    ...
    this.dispatchEvent(
      new CartLinesUpdateEvent({
        action:
          config.action === "change" && quantity > 0 ? "update" : "remove",
        context: "cart",
      ...
      ?.then(async ({ detail }) => {
        ...
        if (cartItemsHtml) {
          const existingKeys = new Set(
            this.refs.cartItemRows?.map((row) => row.dataset.key) ?? [],
          );
